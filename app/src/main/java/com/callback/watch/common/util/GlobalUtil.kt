...
import com.callback.watch.common.extension.edit
import java.util.*
...
object GlobalUtil {
    ...
    val appPackage: String
        get() = WatchApplication.context.packageName
     ...
     */
    val appName: String
        get() = WatchApplication.context.resources.getString(WatchApplication.context.applicationInfo.labelRes)
    ...
    val appVersionName: String
        get() = WatchApplication.context.packageManager.getPackageInfo(appPackage, 0).versionName
    ...
    val appVersionCode: Long
        get() = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            WatchApplication.context.packageManager.getPackageInfo(appPackage, 0).longVersionCode
        } else {
            WatchApplication.context.packageManager.getPackageInfo(appPackage, 0).versionCode.toLong()
        }
    ...
    fun getDeviceSerial(): String {
        if (deviceSerial == null) {
            ...
            if ("google" != appChannel || "samsung" != appChannel) {
                try {
                    deviceId = Settings.Secure.getString(WatchApplication.context.contentResolver, Settings.Secure.ANDROID_ID)
                } catch (e: Exception) {
    ...
    fun getString(resId: Int): String {
        return WatchApplication.context.resources.getString(resId)
    }
    ...
    fun getDimension(resId: Int): Int {
        return WatchApplication.context.resources.getDimensionPixelOffset(resId)
    }
    ...
    fun getResourceId(name: String, type: String): Int {
        return WatchApplication.context.resources.getIdentifier(name, type, appPackage)
    }
    ...
    fun getApplicationMetaData(key: String): String? {
        ...
        try {
            applicationInfo = WatchApplication.context.packageManager.getApplicationInfo(appPackage, PackageManager.GET_META_DATA)
    ...
    fun isInstalled(packageName: String): Boolean {
        val packageInfo: PackageInfo? = try {
            WatchApplication.context.packageManager.getPackageInfo(packageName, 0)
        } catch (e: PackageManager.NameNotFoundException) {
            null
        }
    ...
    fun getAppIcon(): Drawable {
        val packageManager = WatchApplication.context.packageManager
